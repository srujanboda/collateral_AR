...
internal class AndroidARView(
    ...
    private var hitTestFromCenter = false
    private lateinit var animatedGuide: View
    private var featurePointMaterial: Material? = null
    private var featurePointCube: ModelRenderable? = null
...
//                        onDestroy()
    ...
    private fun initializeARView(call: MethodCall, result: MethodChannel.Result) {
        ...
        val argHitTestFromCenter: Boolean? = call.argument<Boolean>("hitTestFromCenter")
        ...
        if (argHitTestFromCenter == true) {
            hitTestFromCenter = true
        } else {
            hitTestFromCenter = false
        }
        ...
        if (argShowFeaturePoints == true) {
            ...
            // Pre-create material and cube for performance
            MaterialFactory.makeOpaqueWithColor(viewContext, Color(android.graphics.Color.YELLOW))
                .thenAccept { material ->
                    featurePointMaterial = material
                    featurePointCube = ShapeFactory.makeCube(Vector3(0.01f, 0.01f, 0.01f), Vector3.zero(), material)
                    featurePointCube?.isShadowCaster = false
                    featurePointCube?.isShadowReceiver = false
                }
        } else {
    ...
    private fun onFrame(frameTime: FrameTime) {
        ...
        if (showFeaturePoints) {
            ...
            if (points.limit() / 4 >= 1 && featurePointCube != null) {
                val numPoints = points.limit() / 4
                val currentChildCount = pointCloudNode.children?.size ?: 0
                
                for (index in 0 until numPoints) {
                    val x = points.get(4 * index)
                    val y = points.get(4 * index + 1)
                    val z = points.get(4 * index + 2)
                    
                    val node = if (index < currentChildCount) {
                        pointCloudNode.children?.get(index)
                    } else {
                        val newNode = Node()
                        newNode.renderable = featurePointCube
                        newNode.setParent(pointCloudNode)
                        newNode
                    }
                    node?.worldPosition = Vector3(x, y, z)
                }
                
                // Remove extra nodes if point cloud shrank
                if (currentChildCount > numPoints) {
                    for (i in (currentChildCount - 1) downTo numPoints) {
                        pointCloudNode.children?.get(i)?.setParent(null)
                    }
                }
            }
            // Release resources
    ...
    private fun onTap(hitTestResult: HitTestResult, motionEvent: MotionEvent?): Boolean {
        ...
        if (motionEvent != null && motionEvent.action == MotionEvent.ACTION_DOWN) {
            if (transformationSystem.selectedNode == null || (!enablePans && !enableRotation)){
                val allHitResults = if (hitTestFromCenter) {
                    val x = arSceneView.width / 2f
                    val y = arSceneView.height / 2f
                    frame?.hitTest(x, y) ?: listOf<HitResult>()
                } else {
                    frame?.hitTest(motionEvent) ?: listOf<HitResult>()
                }
