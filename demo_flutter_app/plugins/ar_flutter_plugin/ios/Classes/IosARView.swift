...
class IosARView: NSObject, FlutterPlatformView, ARSCNViewDelegate, UIGestureRecognizerDelegate, ARSessionDelegate {
    ...
    func initializeARView(arguments: Dictionary<String,Any>, result: FlutterResult){
        // Set hit test from center configuration
        if let configHitTestFromCenter = arguments["hitTestFromCenter"] as? Bool {
            hitTestFromCenter = configHitTestFromCenter
        }
    ...
    @objc func handleTap(_ recognizer: UITapGestureRecognizer) {
        ...
        if (hitTestFromCenter) {
            touchLocation = CGPoint(x: sceneView.bounds.midX, y: sceneView.bounds.midY)
        } else {
